...
- Tasks:
  ...
  - [x] Integrate supertest smoke test pipeline via `npm run test:server`.
...
- Tasks:
  - [x] Introduce ORM layer (Prisma or TypeORM) with schema for products, inventory, and transactions.
  - [x] Add migrations + seed script; connect to postgres service in Docker Compose.
  - [x] Extend tests to cover repository layer using test containers.
...
- Tasks:
  - [x] Build responsive product grid with cart interactions and mocked payment trigger.
  - [x] Fetch data from backend API (dev proxy) with robust loading/error states.
  - [x] Add Vitest/RTL smoke tests for key kiosk flows (`npm run test:client`).
